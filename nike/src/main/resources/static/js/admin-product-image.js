$(document).ready(function() {
    ...
    $(".btn-save-product").on("click", function () {
        ...
        var linkPost = "/api/product-image/delete";
        var pdImInfo = $(this).data("id");
        if(pdImInfo.id) {
            linkPost = "/api/product-image/delelte/" + pdImInfo.id;
        }

        axios.post(linkPost, pdImInfo).then(function(res){
            NProgress.done();
            if(res.data.success) {
                swal(
                    'Good job!',
